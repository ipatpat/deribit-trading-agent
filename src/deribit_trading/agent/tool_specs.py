...
TOOL_DESCRIPTIONS_OVERRIDE: dict[str, str] = {
    ...
    "get_market_snapshot": (
        ...
        "unknown. Default kind='option' (most common); pass 'future' or "
        "'perpetual' to scope; pass empty string '' to include ALL "
        "instrument kinds at once."
    ),
    "get_positions": (
    ...
    "place_order": (
        ...
        "direction ('buy'/'sell'), amount, order_type ('limit'/'market', "
        "default limit), price (required for limit), label (optional). "
        "AMOUNT UNITS: perpetuals & dated futures are inverse contracts — "
        "amount is the number of USD-denominated contracts (BTC contract = "
        "$10 face, ETH contract = $1 face), typically an integer. Options "
        "amount is in underlying-coin units (e.g. BTC for BTC options). "
        "Per-instrument minimum / step varies and is NOT a fixed rule; do "
        "not assume 0.1 or 1 — let Deribit validate. On a -32602 error with "
        "`data.reason` like 'must be a multiple of the minimum order size' "
        "or `data.param` set, read the error data and resubmit at the next "
        "valid amount instead of retrying the same value. Repeated identical "
        "retries waste the user's confirmation taps. "
        ...
        "have echoed the parameters for them to verify. If the user wants "
        "maker-preferred execution or wants the order worked over time, "
        "prefer smart_order instead of a single-shot place_order."
    ),
    "cancel_order": (
    ...
    "smart_order": (
        "Create a managed execution order routed by intent (intent-router: "
        "maker-first → escalating ladder Lv0→Lv4). EACH CALL TRIGGERS A USER "
        "CONFIRMATION CARD before the order is created. "
        "Required params: instrument_name, direction ('buy'/'sell'), amount. "
        "intent: 'standard' (default — patient maker at Lv0, escalates via "
        "Lv1 mid → Lv2 opposite top → Lv3 IOC → Lv4 market as patience "
        "burns) or 'urgent' (skip patience, fire Lv3 IOC immediately). "
        "Optional overrides (dict): t_patience_ms (patience budget in ms, "
        "default 30000), max_cross_levels (IOC cross-depth cap, default 1), "
        "price_limit_pct (perp, e.g. 0.003 = ±0.3%) OR price_limit_ticks "
        "(option, e.g. 5) OR price_limit_iv (option, e.g. 0.02 = ±2 vols), "
        "prefer_maker (bool). Returns {id, state, current_level, ...}. "
        "AMOUNT rules match place_order — per-instrument min / step varies; "
        "if the response has state=failed with last_error containing "
        "'-32602' or 'minimum order size', read data.reason / data.param "
        "and resubmit at a valid amount instead of retrying the same value. "
        "Use this when the user wants execution-quality optimisation ('work "
        "this order to get a maker fill', 'fill in the next minute without "
        "moving the book'). For an immediate single-shot fill, use "
        "place_order instead. "
        "Example: smart_order(instrument_name='BTC-PERPETUAL', "
        "direction='buy', amount=10, intent='standard', "
        "overrides={'t_patience_ms': 45000})."
    ),
    "cancel_smart_order": (
