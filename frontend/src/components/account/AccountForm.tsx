...
interface AccountFormProps {
  /** When set, the form is in edit mode; only alias + secret are mutable. */
  account?: AccountSummary | null;
  endpoints: EndpointInfo[];
  /** Show endpoint radio. Welcome always true; ignored when editing (endpoint is locked). */
  showEnvSwitcher?: boolean;
  /** Primary submit button label. Defaults based on edit mode. */
  primaryLabel?: string;
  /** Show the secondary Cancel button. */
  showCancel?: boolean;
  /** Show the inline Test connection control. */
...
function AccountForm({
  ...
  // Re-seed fields when the target account or endpoint list changes.
  useEffect(() => {
    if (account) {
      ...
      setAlias('');
      // Default to Paper Trade for safety. Welcome onboarding requires this.
      const paper = endpoints.find((ep) => !ep.is_production);
      setEndpoint((paper?.id as EndpointId) ?? (endpoints[0]?.id as EndpointId) ?? 'deribit_testnet');
      setClientId('');
      setClientSecret('');
    }
    setTestResult(null);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="pt-2 flex gap-2 justify-end">
        ...
        >
          {saving ? 'Saving...' : resolvedPrimaryLabel}
        </button>
      </div>
    </div>
  );
